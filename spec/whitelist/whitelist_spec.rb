...
RSpec.describe Whitelist do
  ...
  it "constructs a disjunction predicate" do
    ...
    expect(conj_predicate.call(['1', 'foo', 'bar'])).to eq(true)
    expect(conj_predicate.call(['3', 'foo', 'bar'])).to eq(true)
    expect(conj_predicate.call(['1', 'foo', 'whatever'])).to eq(false)
    expect(conj_predicate.call(['1', 'whatever', 'bar'])).to eq(false)
    expect(conj_predicate.call(['3', 'whatever', 'bar'])).to eq(true)
  end
  ...
  it "constructs a value-testing predicate" do
    ...
    expect(test_predicate.call(['1', 'foo', 'bar'])).to eq(true)
    expect(test_predicate.call(['3', 'foo', 'bar'])).to eq(true)
    expect(test_predicate.call(['1', 'foo', 'whatever'])).to eq(true)
    expect(test_predicate.call(['1', 'whatever', 'bar'])).to eq(false)
  end
